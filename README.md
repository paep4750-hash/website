...
<head>
  ...
  <title>CYBER HUB - Roblox Script Store</title>
...
<body class="bg-slate-950 text-slate-100 font-sans min-h-screen flex flex-col justify-between selection:bg-purple-600 selection:text-white">
  ...
  <header class="sticky top-0 z-40 bg-slate-900/90 backdrop-blur-md border-b border-slate-800 px-4 py-3">
    <div class="max-w-4xl mx-auto flex items-center justify-between">
      ...
      <div id="nav-actions" class="text-[10px] text-slate-400">🟢 ระบบออฟไลน์ความปลอดภัยสูง</div>
    </div>
  </header>
    ...
    <div id="view-gate" class="max-w-sm mx-auto mt-4">
      <div class="bg-slate-900/80 border border-slate-800 rounded-xl p-5 shadow-xl space-y-4">
        <div class="text-center">
          ...
          <p class="text-[10px] text-slate-400">คีย์ผู้พัฒนาได้รับการเข้ารหัสความปลอดภัยระดับสูงสุด</p>
        </div>
  ...
  <script>
    // การพรางลายเซ็นรหัสแอดมิน เพื่อความปลอดภัยสูงสุดไม่ให้ถูกมองเห็นเป็นตัวเลขตรงๆ
    const verifyAdmin = (key) => btoa(key) === "MTU0NjY2NzU1NDY4NDQ3NzY=" || btoa(key) === "MTUzMjI3NzUz";

    const state = { view: 'gate', auth: false, admin: false, keys: [{ key: 'KEY-FREE-9999', expires: Date.now() + 86400000 }], activeGame: 'All', scripts: [
      ...
      { id: 2, title: 'Pet Simulator 99 Infinite Coins', game: 'Pet Simulator 99', category: 'Item Farm', description: 'ระบบออโต้ฟาร์มเงิน ตีเพชร และลูปเปิดไข่อัตโนมัติ', code: 'loadstring(game:HttpGet("https://raw.githubusercontent.com/CyberHubRoblox/main/petsim99.lua"))()' }
    ]};
    ...
    const showToast = (msg, err = false) => {
      ...
      el.className = `px-3 py-1.5 rounded text-[10px] font-bold border ${err ? 'bg-red-950 text-red-300 border-red-500/20' : 'bg-emerald-950 text-emerald-300 border-emerald-500/20'}`;
    ...
    const renderNav = () => {
      ...
      container.innerHTML = `
        ...
        ${state.admin ? `<button onclick="setView('admin')" class="px-2 py-1 rounded text-[10px] font-bold ${state.view === 'admin' ? 'text-indigo-400 bg-indigo-500/10' : 'text-slate-400'}">จัดการแอดมิน</button>` : ''}
    ...
    const verifyKey = () => {
      ...
      if (verifyAdmin(input)) {
        state.auth = true; state.admin = true; setView('hub'); showToast('สิทธิ์ผู้พัฒนาปลดล็อกเรียบร้อย!'); initHub();
      } else if (state.keys.some(k => k.key === input && k.expires > Date.now())) {
        state.auth = true; state.admin = false; setView('hub'); showToast('ผ่านคีย์สำเร็จ!'); initHub();
    ...
    const loginAdmin = () => {
      ...
      if (verifyAdmin(input)) {
        state.auth = true; state.admin = true; setView('admin'); showToast('ยินดีต้อนรับแอดมิน!'); initHub();
    ...
    const renderFilters = () => {
      ...
      container.innerHTML = list.map(g => `<button onclick="setGame('${g}')" class="px-2 py-0.5 text-[9px] rounded font-bold ${state.activeGame === g ? 'bg-purple-600 text-white' : 'bg-slate-900 text-slate-400'}">${g === 'All' ? 'ทั้งหมด' : g}</button>`).join('');
    };
    ...
    const renderHub = () => {
      ...
      grid.innerHTML = list.length === 0 ? '<p class="text-[10px] text-slate-500 text-center py-4">ไม่พบสคริปต์ในระบบ</p>' : list.map(s => `
        <div class="bg-slate-900/50 border border-slate-850 p-3 rounded-lg flex flex-col justify-between hover:bg-slate-900">
          ...
          <div class="flex justify-end gap-1 pt-3 border-t border-slate-800/40 mt-3">
            <button onclick="viewScript(${s.id})" class="bg-slate-850 text-[9px] px-2 py-1 text-slate-200 rounded">ดูสคริปต์</button>
            <button onclick="copyCode(${s.id})" class="bg-purple-600 text-[9px] px-2 py-1 text-white rounded font-bold">📋 ก็อป</button>
          </div>
        </div>
      `).join('');
    };
    ...
    const renderAdmin = () => {
      ...
      document.getElementById('admin-list').innerHTML = state.scripts.map(s => `
        <tr class="border-b border-slate-800/40 text-[10px]"><td class="p-2 text-white max-w-[120px] truncate">${s.title}</td><td>${s.game}</td><td class="text-right"><button onclick="editScript(${s.id})" class="text-indigo-400 px-1 font-bold">✏️</button><button onclick="deleteScript(${s.id})" class="text-red-400 px-1 font-bold">🗑️</button></td></tr>
      `).join('');
    ...
    const saveScript = (e) => {
      ...
      if (id) {
        ...
        state.scripts.unshift({ id: Date.now(), title, game, category, description, code });
      }
    ...
    const deleteScript = (id) => { if(confirm('คุณต้องการลบสคริปต์นี้ใช่หรือไม่?')) { state.scripts = state.scripts.filter(s => s.id !== id); showToast('ลบสคริปต์เรียบร้อย'); initHub(); }};
  </script>
</body>
</html>
